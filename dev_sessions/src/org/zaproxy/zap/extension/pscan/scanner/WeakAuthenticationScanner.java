...
public class WeakAuthenticationScanner extends PluginPassiveScanner {
		...
		for (Iterator<String> i = headers.iterator(); i.hasNext();) {
			...
			{				
				...
				if ( authMechanism.toLowerCase(Locale.ENGLISH).equals ("basic") ) {
					...
					extraInfo = getString("weakauthentication.alert.basicauth.extrainfo", 
				...
				if ( authMechanism.toLowerCase(Locale.ENGLISH).equals ("digest") ) {
					...
					extraInfo = getString("weakauthentication.alert.digestauth.extrainfo", 
							method, uri, authMechanism, username, authValues[1]);
							
					digestInfo=authValues[1]; //info to output in the logging message.
					}
						
    			String attack = getString("weakauthentication.alert.attack");
    			String vulnname=getString("weakauthentication.name");
    			String vulndesc=getString("weakauthentication.desc");
    			String vulnsoln=getString("weakauthentication.soln");
    			String vulnrefs=getString("weakauthentication.refs");
    			
    			//raise the alert, now that we have all the detail on it.
    			Alert alert = new Alert(getId(), alertRisk, alertLevel, vulnname);
    			alert.setDetail(vulndesc, 
			    		uri,
    			...
    			//and log it
				String logMessage = getString ("weakauthentication.alert.logmessage", 
							method, uri, authMechanism, username, (digestInfo!=null)?digestInfo:password);
    			log.info(logMessage);
	...
	@Override
	public void scanHttpResponseReceive(HttpMessage arg0, int arg1, Source arg2) {
		//not interested in the response. return
		return;
