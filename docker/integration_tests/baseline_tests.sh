...
echo "TEST: Baseline test 1 (vs zaproxy.com)"
/zap/zap-baseline.py -s -t https://www.zaproxy.com/ > /zap/wrk/output/baseline1.out
check_results "baseline1" $? 2

echo
echo "TEST: Baseline test 2 (vs zaproxy.com with INFO/WARN/FAIL set)"
/zap/zap-baseline.py -s -t https://www.zaproxy.com/ --auto -c configs/baseline2.conf > /zap/wrk/output/baseline2.out
check_results "baseline2" $? 1

echo
echo "TEST: Baseline test 3 (vs zaproxy.com with INFO/WARN/FAIL and OUTOFSCOPE set via file)"
/zap/zap-baseline.py -s -t https://www.zaproxy.com/ -c configs/baseline3.conf > /zap/wrk/output/baseline3.out
check_results "baseline3" $? 1

echo
echo "TEST: Baseline test 4 (vs zaproxy.com with INFO/WARN/FAIL and OUTOFSCOPE set via URL)"
/zap/zap-baseline.py -s -t https://www.zaproxy.com/ -u https://raw.githubusercontent.com/zaproxy/zaproxy/main/docker/integration_tests/configs/baseline3.conf > /zap/wrk/output/baseline4.out
...
/zap/zap-baseline.py -s -t https://www.zaproxy.com/ --autooff > /zap/wrk/output/baseline1-orig.out
