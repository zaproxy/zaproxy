...
def main():
    ...
    parser.add_argument("-S", "--safe", action='store_true', default=False,
                        help="safe mode this will skip the active scan and perform a baseline scan")
    ...
    parser.add_argument("--schema", default='',
                        help="GraphQL schema location, URL or file, e.g. https://www.example.com/schema.graphqls\n")
    ...
    try:
        ...
        # Enable scripts
        zap.script.load('Alert_on_HTTP_Response_Code_Errors.js', 'httpsender', 'Oracle Nashorn',
        ...
        zap.script.load('Alert_on_Unexpected_Content_Types.js', 'httpsender', 'Oracle Nashorn',
        ...
        if format == 'openapi':
            ...
            if target_url:
                ...
                if host_override:
                    ...
                    logging.info(
                        'Using host override, new target: {0}'.format(target))
            else:
            ...
            if target_url:
                ...
                res = zap._request(
                    zap.base + 'soap/action/importFile/', {'file': base_dir + target_file})
                urls = zap.core.urls()
                if len(urls) > 0:
                    # Choose the first one - will be striping off the path below
                    target = urls[0]
            ...
            if schema:
                ...
                res = zap.graphql.import_file(
                    target, base_dir + schema) if schema_file else zap.graphql.import_url(target, schema_url)
            else:
