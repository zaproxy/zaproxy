...
def load_custom_hooks(hooks_file=None):
    ...
    global zap_hooks
    hooks_file = (
        hooks_file if hooks_file else os.environ.get("ZAP_HOOKS", "~/.zap_hooks.py")
    )
    ...
    if not os.path.exists(hooks_file):
        logging.warning(
            "Could not find custom hooks file at %s " % os.path.abspath(hooks_file)
        )
        return
...
def trigger_hook(name, *args, **kwargs):
    ...
    if not response:
        ...
        return args
    elif (isinstance(response, list) or isinstance(response, tuple)) and len(
        response
    ) != arg_length:
        return args
    return response
...
def inc_info_rules(config_dict, key, inc_extra):
    return ((key in config_dict) and config_dict[key] == "INFO") or (
        (key not in config_dict) and inc_extra
    )


def inc_warn_rules(config_dict, key, inc_extra):
    return ((key in config_dict) and config_dict[key] == "WARN") or (
        (key not in config_dict) and inc_extra
    )
...
def running_in_docker():
    return (
        ...
        or os.environ.get("IS_CONTAINERIZED") == "true"
    )
...
def ipaddress_for_cid(cid):
    insp_output = (
        subprocess.check_output(["docker", "inspect", cid]).strip().decode("utf-8")
    )
...
def zap_ajax_spider(zap, target, max_time):
    ...
    while zap.ajaxSpider.status == "running":
        logging.debug(
            "Ajax Spider running, found urls: %s", zap.ajaxSpider.number_of_results
        )
        time.sleep(5)
...
def zap_active_scan(zap, target, policy):
    if scan_user:
        logging.debug(
            "Active Scan %s with policy %s as user %s",
            target,
            policy,
            scan_user["name"],
        )
...
def check_zap_client_version():
    ...
    if (
        current_version
        and latest_version
        and parse_version(current_version) < parse_version(latest_version)
    ):
