...
class pnh(object):
	...
	def monitor(self, id, message, apikey=''):
		...
		return next(six.itervalues(
			self.zap._request(self.zap.base + 'pnh/action/monitor/', {'id': id, 'message': message, 'apikey':
                apikey})))
	...
	def oracle(self, id, apikey=''):
		...
		"""
		return next(
			six.itervalues(self.zap._request(self.zap.base + 'pnh/action/oracle/', {'id': id, 'apikey': apikey})))
	...
	def start_monitoring(self, url, apikey=''):
		...
		"""
		return next(six.itervalues(
			self.zap._request(self.zap.base + 'pnh/action/startMonitoring/', {'url': url, 'apikey': apikey})))
	...
	def stop_monitoring(self, id, apikey=''):
		...
		"""
		return next(six.itervalues(
			self.zap._request(self.zap.base + 'pnh/action/stopMonitoring/', {'id': id, 'apikey': apikey})))
