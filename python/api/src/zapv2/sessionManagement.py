...
class sessionManagement(object):
    ...
    def get_supported_session_management_methods(self):
        return next(six.itervalues(
            self.zap._request(self.zap.base + 'sessionManagement/view/getSupportedSessionManagementMethods/')))

    def get_session_management_method_config_params(self, methodname):
        return next(six.itervalues(
            self.zap._request(self.zap.base + 'sessionManagement/view/getSessionManagementMethodConfigParams/',
                              {'methodName': methodname})))

    def get_session_management_method(self, contextid):
        return next(six.itervalues(
            self.zap._request(self.zap.base + 'sessionManagement/view/getSessionManagementMethod/',
                              {'contextId': contextid})))
    ...
    def set_session_management_method(self, contextid, methodname, methodconfigparams=None, apikey=''):
        ...
        return next(six.itervalues(
            self.zap._request(self.zap.base + 'sessionManagement/action/setSessionManagementMethod/', params)))
