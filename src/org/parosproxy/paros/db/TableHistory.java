...
public interface TableHistory extends DatabaseListener {
	...
	RecordHistory read(int historyId)
			throws HttpMalformedHeaderException, DatabaseException;

	RecordHistory write(long sessionId, int histType,
			HttpMessage msg) throws HttpMalformedHeaderException, DatabaseException;
	 ...
	 */
	List<Integer> getHistoryIds(long sessionId)
	 ...
	 */
	List<Integer> getHistoryIdsOfHistType(long sessionId,
	...
	List<Integer> getHistoryList(long sessionId, int histType,
			String filter, boolean isRequest) throws DatabaseException;

	void deleteHistorySession(long sessionId)
			throws DatabaseException;

	void deleteHistoryType(long sessionId, int historyType)
			throws DatabaseException;
	 ...
	 * @since 2.3.0
	 */
	void delete(List<Integer> ids, int batchSize)
			throws DatabaseException;
	...
	boolean containsURI(long sessionId, int historyType,
			String method, String uri, byte[] body) throws DatabaseException;

	RecordHistory getHistoryCache(HistoryReference ref,
			HttpMessage reqMsg) throws DatabaseException,
