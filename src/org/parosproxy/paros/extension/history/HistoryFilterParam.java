...
public class HistoryFilterParam extends AbstractParam {
    ...
    protected void parse() {
        try {
            List<HierarchicalConfiguration> filtersConfig = ((HierarchicalConfiguration) getConfig()).configurationsAt(ALL_FILTERS_KEY);
            ...
            for (HierarchicalConfiguration filterConfig : filtersConfig) {
                ...
                filter.setUrlIncPatternList(filterConfig.getStringArray(FILTER_URL_INC_PATTERN_LIST_KEY));
                filter.setUrlExcPatternList(filterConfig.getStringArray(FILTER_URL_EXC_PATTERN_LIST_KEY));
                filter.setMethods(asStringList(filterConfig.getStringArray(FILTER_METHOD_LIST_KEY)));
                filter.setCodes(asIntList(filterConfig.getStringArray(FILTER_CODE_LIST_KEY)));
                filter.setReliabilities(asStringList(filterConfig.getStringArray(FILTER_CONFIDENCE_LIST_KEY)));
                ...
                filter.setStartTimeSentInMs(asOptional(filterConfig.getLong(FILTER_START_TIME_SENT_KEY, null)));
                filters.add(filter);
    ...
    private void saveRules() {
        ...
        for (int i = 0, size = filters.size(); i < size; ++i) {
            ...
            getConfig().setProperty(elementBaseKey + FILTER_ENABLED_KEY, Boolean.valueOf(filter.isEnabled()));
            getConfig().setProperty(elementBaseKey + FILTER_NAME_KEY, filter.getName());
            getConfig().setProperty(elementBaseKey + FILTER_URL_INC_PATTERN_LIST_KEY, filter.getUrlIncPatternValueList());
            getConfig().setProperty(elementBaseKey + FILTER_URL_EXC_PATTERN_LIST_KEY, filter.getUrlExcPatternValueList());
            getConfig().setProperty(elementBaseKey + FILTER_METHOD_LIST_KEY, asStringArrayFromStringList(filter.getMethodList()));
            getConfig().setProperty(elementBaseKey + FILTER_CODE_LIST_KEY, asStringArrayFromIntList(filter.getCodeList()));
            getConfig().setProperty(elementBaseKey + FILTER_CONFIDENCE_LIST_KEY, asStringArrayFromStringList(filter.getConfidenceList()));
            getConfig().setProperty(elementBaseKey + FILTER_RISKS_LIST_KEY, asStringArrayFromStringList(filter.getRiskList()));
            getConfig().setProperty(elementBaseKey + FILTER_TAG_LIST_KEY, asStringArrayFromStringList(filter.getTagList()));
            getConfig().setProperty(elementBaseKey + FILTER_NOTE_KEY, filter.getNote());
            if(filter.getStartTimeSentInMs().isPresent()){
                getConfig().setProperty(elementBaseKey + FILTER_START_TIME_SENT_KEY, filter.getStartTimeSentInMs().get());
            }
        }
    ...
    private String[] asStringArrayFromIntList(List<Integer> values) {
        return asStringArrayFromStringList(values
                .stream()
                .map(v -> v.toString())
    ...
    private <T> List<T> asList(String[] stringArray, java.util.function.Function<String, T> convert) {
