...
public class OptionsHistoryFilterPanel extends AbstractParamPanel {
    ...
    public static final String PANEL_NAME = Constant.messages.getString("history.filter.options.title");
    ...
    private void initialize() {
        ...
        historyFilterTablePanel = new HistoryFilterMultipleOptionsTablePanel(getHistoryFilterTableModel());
    ...
    public void validateParam(Object obj) throws Exception {
        List<String> filters = getHistoryFilterTableModel()
                .getElements()
                .stream()
                .map(f -> f.getName())
        ...
        if(filters.size() != filters.stream().distinct().count()){
            throw new IllegalArgumentException(
                    Constant.messages.getString(
                            "history.filter.save.dialog.unique"));
        }
    }
    ...
    private static class HistoryFilterMultipleOptionsTablePanel extends AbstractMultipleOptionsTablePanel<HistoryFilter> {

        private static final long serialVersionUID = -115340627058929308L;

        private static final String REMOVE_DIALOG_TITLE = Constant.messages.getString("history.filter.options.dialog.remove.title");
        private static final String REMOVE_DIALOG_TEXT = Constant.messages.getString("history.filter.options.dialog.remove.text");

        private static final String REMOVE_DIALOG_CONFIRM_BUTTON_LABEL = Constant.messages.getString("history.filter.options.dialog.remove.button.confirm");
        private static final String REMOVE_DIALOG_CANCEL_BUTTON_LABEL = Constant.messages.getString("history.filter.options.dialog.remove.button.cancel");

        private static final String REMOVE_DIALOG_CHECKBOX_LABEL = Constant.messages.getString("history.filter.options.dialog.checkbox.label");
        ...
        public HistoryFilterMultipleOptionsTablePanel(HistoryFilterTableModel model) {
            ...
            getTable().setHorizontalScrollEnabled(true); // FilterDescription could be very wide, so turn on horiz scroll
