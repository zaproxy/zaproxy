	...
	public JSON handleApiAction(String name, JSONObject params) throws ApiException {
		...
		if (ACTION_SHUTDOWN.equals(name)) {
			...
			if (sessionName == null || sessionName.length() == 0) {
				throw new ApiException(ApiException.Type.MISSING_PARAMETER, ACTION_SESSION_PARAM_NAME);
			}
			session.setSessionName(name);
			if ( ! sessionName.endsWith(".session")) {
				sessionName = sessionName + ".session";
			}
			String filename = Model.getSingleton().getOptionsParam().getUserDirectory() + File.separator + sessionName;
			if (new File(filename).exists()) {
				throw new ApiException(ApiException.Type.INTERNAL_ERROR, filename);
			}
			try {
	...
	private List<Alert> getAlerts() throws ApiException {
	    ...
	    try {
			...
			for (int i=0; i<v.size(); i++) {
			    ...
			    if (alert.getReliability() != Alert.FALSE_POSITIVE && ! alerts.contains(alert)) {
                	alerts.add(alert);
                }
			}
		    return alerts;
