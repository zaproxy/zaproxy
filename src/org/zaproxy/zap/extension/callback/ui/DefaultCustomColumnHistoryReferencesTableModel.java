...
public class DefaultCustomColumnHistoryReferencesTableModel<T extends DefaultHistoryReferencesTableEntry> extends DefaultHistoryReferencesTableModel  {
     ...
     * @param customColumns Provide the implementations of the custom columns you passed to the {@code columns} parameter.
     *                      When the count does not match then empty columns are created on that particular index.
     */
    public DefaultCustomColumnHistoryReferencesTableModel(final Column[] columns, final ArrayList<CustomColumn<T>> customColumns,  Class<T> type) {
        super(columns);
        this.type = type;
        this.customColumns = createCustomColumnMap(columns, customColumns);
    }

    private Map<Integer, CustomColumn<T>> createCustomColumnMap(Column[] columns, ArrayList<CustomColumn<T>> customColumns){
        Map<Integer, CustomColumn<T>> customColumnMap = new HashMap<>();

        int customColumnIndex = 0;
        for (int i = 0; i < columns.length; i++) {
            if(columns[i] == Column.CUSTOM){
                CustomColumn<T> customColumn = getCustomColumnIfExists(customColumns, customColumnIndex);
    ...
    private CustomColumn<T> getCustomColumnIfExists(ArrayList<CustomColumn<T>> customColumns, Integer index) {
