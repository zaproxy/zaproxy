...
public class ExtensionReauth extends ExtensionAdaptor implements HttpSenderListener, SessionChangedListener, ContextPanelFactory, ContextDataFactory {
	...
	protected void setLoginRequest(int contextId, String url, String postData) throws Exception {
		if (url == null || url.length() == 0) {
			...
			if (isSiteNodeMatch(sn, uri, postData)) {
    ...
    private static boolean isSiteNodeMatch(SiteNode sn, URI uri, String postData) throws Exception {
        if (sn == null) {
            return false;
        }
        ...
        if (!httpMessage.getUrlParams().equals(session.getUrlParams(uri))) {
            return false;
        }
