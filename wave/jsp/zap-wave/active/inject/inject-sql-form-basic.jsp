...
<head>
<title>OWASP ZAP WAVE - Inject SQL Form basic</title>
</head>
<body>
<H2>OWASP ZAP WAVE - Simple SQL injection in a Form parameter</H2>
<H3>Description</H3>
The 'name' parameter in the form is put in a db table without being sanitised and not using a prepared statement and so is vulnerable to an SQL injection attack.<br>
...
<%
	...
	try {
        ...
        ResultSet rs = c.getMetaData().getColumns(null, null, "INJECT_SQL_FORM_BASIC", "NAME");
        if (!rs.next()) {
		// Create the schema
		PreparedStatement stmt = c.prepareStatement("CREATE CACHED TABLE INJECT_SQL_FORM_BASIC (NAME varchar default '')");
		stmt.execute();
	}
        rs.close();
	...
	if (request.getMethod().equals("POST") && name != null) {
		...
		stmt.execute("INSERT INTO INJECT_SQL_FORM_BASIC (NAME) VALUES ('" + name + "')");
	}
	c.close();
%>
