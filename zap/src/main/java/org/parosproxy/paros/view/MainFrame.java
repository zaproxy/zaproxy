...
public class MainFrame extends AbstractFrame {
    ...
    private static final String TAB_SIDE_BY_SIDE_VIEW_TOOL_TIP =
            Constant.messages.getString("view.toolbar.messagePanelsPosition.tabSideBySide");
    private static final String ABOVE_VIEW_TOOL_TIP =
            Constant.messages.getString("view.toolbar.messagePanelsPosition.above");
    private static final String DISABLED_ABOVE_VIEW_TOOL_TIP =
            Constant.messages.getString("view.toolbar.messagePanelsPosition.above.disabled");
    private static final String SIDE_BY_SIDE_VIEW_TOOL_TIP =
            Constant.messages.getString("view.toolbar.messagePanelsPosition.sideBySide");
    private static final String DISABLED_SIDE_BY_SIDE_VIEW_TOOL_TIP =
            Constant.messages.getString("view.toolbar.messagePanelsPosition.sideBySide.disabled");
    ...
    /**
     * The {@code ZapToggleButton} that sets the response panel position {@link
    ...
    private JToggleButton getShowTabIconNamesButton() {
        if (showTabIconNamesButton == null) {
            ...
            showTabIconNamesButton.setToolTipText(
                    Constant.messages.getString("view.toolbar.showNames"));
            showTabIconNamesButton.setSelectedIcon(
                    new ImageIcon(
                            WorkbenchPanel.class.getResource("/resource/icon/ui_tab_text.png")));
            showTabIconNamesButton.setSelectedToolTipText(
                    Constant.messages.getString("view.toolbar.showIcons"));
            showTabIconNamesButton.setSelected(
                    Model.getSingleton().getOptionsParam().getViewParam().getShowTabNames());
            DisplayUtils.scaleIcon(showTabIconNamesButton);

            showTabIconNamesButton.addActionListener(
                    new java.awt.event.ActionListener() {
                        ...
                        public void actionPerformed(ActionEvent evt) {
                            ...
                            Model.getSingleton()
                                    ...
                                    .getViewParam()
                                    .setShowTabNames(showTabNames);
                            Stats.incCounter("stats.ui.toolbar.button.tabs.namesicons");
                            try {
                                Model.getSingleton()
                                        .getOptionsParam()
                                        .getViewParam()
    ...
    private ZapToggleButton getTabsResponsePanelPositionButton() {
        if (tabsResponsePanelPositionButton == null) {
            tabsResponsePanelPositionButton =
                    new ZapToggleButton(
    ...
    private ZapToggleButton getTabSideBySideResponsePanelPositionButton() {
        if (tabSideBySideResponsePanelPositionButton == null) {
            tabSideBySideResponsePanelPositionButton =
                    new ZapToggleButton(
                            new SetResponsePanelPositionAction(
                                    WorkbenchPanel.class.getResource(
    ...
    private ZapToggleButton getPanelsResponsePanelPositionButton() {
        if (panelsResponsePanelPositionButton == null) {
            panelsResponsePanelPositionButton =
                    new ZapToggleButton(
                            new SetResponsePanelPositionAction(
                                    WorkbenchPanel.class.getResource(
    ...
    private ZapToggleButton getAboveResponsePanelPositionButton() {
        if (aboveResponsePanelPositionButton == null) {
            aboveResponsePanelPositionButton =
                    new ZapToggleButton(
                            new SetResponsePanelPositionAction(
                                    WorkbenchPanel.class.getResource(
     ...
     *
     * <p>It controls the visibility of the main tool bar, the layout and response panel position of
     * the workbench panel and if the tabs should display the panels' names.
     *
     * @since 2.5.0
    ...
    public void applyViewOptions() {
        ...
        setWorkbenchLayout(
                WorkbenchPanel.Layout.getLayout(options.getViewParam().getDisplayOption()));

        WorkbenchPanel.ResponsePanelPosition position =
                WorkbenchPanel.ResponsePanelPosition.TABS_SIDE_BY_SIDE;
        try {
            position =
                    WorkbenchPanel.ResponsePanelPosition.valueOf(
     ...
     *
     * <p>If the layout is already set no further action is taken, otherwise updates the main tool
     * bar buttons, the workbench panel and the configurations file.
     *
    ...
    /**
     * Sets the position of the response panel. Should be considered a hint, not all workbench
     * layouts might use this setting.
     *
     * <p>If the position is already set no further action is taken, otherwise updates the main tool
     * bar buttons, the workbench panel and the configurations file.
     *
     ...
     *
     * <p>The actual title set is the current session name, then the filename of the session (if
     * persisted), followed by the program name and version.
     *
     ...
     * Sets the title of the main window.
     *
     * <p>The actual title set is the given {@code title} followed by the program name and version.
     *
     * @see Constant#PROGRAM_NAME
     * @see Constant#PROGRAM_VERSION
    ...
    private class ChangeWorkbenchLayoutAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            Stats.incCounter("stats.ui.toolbar.button.layout." + layout.name());
        }
    }
    ...
    private class SetResponsePanelPositionAction extends AbstractAction {
        ...
        public SetResponsePanelPositionAction(
                URL iconLocation, WorkbenchPanel.ResponsePanelPosition position) {
        ...
        public void actionPerformed(ActionEvent e) {
            setResponsePanelPosition(position);
            Stats.incCounter("stats.ui.toolbar.button.response." + position.name());
        }
    }
}
