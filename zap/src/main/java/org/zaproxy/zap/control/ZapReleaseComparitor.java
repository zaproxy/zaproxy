    ...
    public int compare(ZapRelease zr1, ZapRelease zr2) {
        ...
        for (int i = 0; i < zr1Array.length; i++) {
            if (i >= zr2Array.length) {
                // Equal up to now, zr1 longer so more recent (e.g. 2.0.0.1 is more recent that 2.0.0)
                return 1;
            }
            ...
            if (zr1Int != null) {
                if (zr2Int != null) {
                    ...
                    if (zr2Array[i].equals(Constant.ALPHA_VERSION)
                        ...
                        // zr2 element alpha or beta, so before any number (e.g. 2.0.0 is more recent
                        // than 2.0.alpha)
                        return 1;
                    } else {
                ...
                if (zr1Array[i].equals(Constant.ALPHA_VERSION)
                    ...
                    // zr1 element alpha or beta, so before any number (e.g. 2.0.0 is more recent than
                    // 2.0.alpha)
                    return -1;
                } else {
