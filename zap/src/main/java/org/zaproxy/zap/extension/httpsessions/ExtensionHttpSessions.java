 ...
 * of them.
 *
 * <p>Whenever referring to a particular site, the string that is identifying it is constructed from
 * the site's URI and has to follow these rules:
 *
 * <ul>
 *   <li>no leading protocol (e.g. 'http'), colon (':') and double slashes ('//')
 *   <li>the port is added in the end after colon
 *   <li>lower-case
 * </ul>
 *
 * An example of a method performing these changes on an URI is {@link
 * ScanPanel#cleanSiteName(String, boolean)} .
 */
public class ExtensionHttpSessions extends ExtensionAdaptor
        implements SessionChangedListener, SiteMapListener, HttpSenderListener {
    ...
    public void hook(ExtensionHook extensionHook) {
        ...
        if (getView() != null) {
            ...
            if (Control.getSingleton().getExtensionLoader().getExtension(ExtensionSearch.class)
                    != null) {
    ...
    private PopupMenuItemCopySessionToken getPopupMenuItemCopySessionToken() {
        if (popupMenuItemCopySessionToken == null) {
            popupMenuItemCopySessionToken =
                    new PopupMenuItemCopySessionToken(getHttpSessionsPanel());
        }
     ...
     * site.
     *
     * @param site the site. This parameter has to be formed as defined in the {@link
     *     ExtensionHttpSessions} class documentation.
     * @param token the token
    ...
    private boolean isRemovedDefaultSessionToken(String site, String token) {
        if (removedDefaultTokens == null) return false;
        HashSet<String> removed = removedDefaultTokens.get(site);
        if (removed == null || !removed.contains(token)) return false;
        return true;
    }
    ...
    private void unmarkRemovedDefaultSessionToken(String site, String token) {
        if (removedDefaultTokens == null) return;
        HashSet<String> removed = removedDefaultTokens.get(site);
     ...
     *
     * @param site the site. This parameter has to be formed as defined in the {@link
     *     ExtensionHttpSessions} class documentation. However, if the protocol is missing, a
     *     default protocol of 80 is used.
     * @param token the token
    ...
    public boolean isSessionToken(String site, String token) {
        ...
        if (siteTokens == null) return false;
     ...
     *
     * <p>All the existing sessions are cleaned up:
     *
     * <ul>
     *   <li>if there are no more session tokens, all session are deleted
     *   <li>in every existing session, the value for the deleted token is removed
     *   <li>if there is a session with no values for the remaining session tokens, it is deleted
     *   <li>if, after deletion, there are duplicate sessions, they are merged
     * </ul>
     *
     * @param site the site. This parameter has to be formed as defined in the {@link
     *     ExtensionHttpSessions} class documentation. However, if the protocol is missing, a
    ...
    public void removeHttpSessionToken(String site, String token) {
        ...
        if (siteTokens != null) {
            ...
            if (siteTokens.isEmpty()) sessionTokens.remove(site);
    ...
    public List<HttpSession> getHttpSessionsForContext(Context context) {
        ...
        synchronized (sessionLock) {
            for (Entry<String, HttpSessionsSite> e : this.sessions.entrySet()) {
                ...
                if (context.isInContext(siteName)) sessions.addAll(e.getValue().getHttpSessions());
            }
        }
    ...
    public void onHttpRequestSend(HttpMessage msg, int initiator, HttpSender sender) {
        ...
        if (getParam().isEnabledProxyOnly() && initiator != HttpSender.PROXY_INITIATOR) return;
        ...
        for (HttpCookie cookie : requestCookies) {
            ...
            if (this.isDefaultSessionToken(cookie.getName())
                    && !this.isSessionToken(site, cookie.getName())
