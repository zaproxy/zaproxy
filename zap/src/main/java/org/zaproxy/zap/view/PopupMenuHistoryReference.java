...
/**
 * @deprecated (2.3.0) Superseded by {@link PopupMenuItemHistoryReferenceContainer}. It will be
...
public abstract class PopupMenuHistoryReference extends ExtensionPopupMenuItem {
                        ...
                        try {
                            if (multiSelect) {
                                ...
                                if (ref != null) {
                                    ...
                                    log.error(
                                            "PopupMenuHistoryReference invoker "
                                                    + lastInvoker
                                                    + " failed to get history ref");
                                }
                            }
                        } catch (Exception e2) {
    ...
    public boolean isEnableForComponent(Component invoker) {
            ...
            if (tree.getLastSelectedPathComponent() != null) {
                if (tree.getSelectionCount() > 1) {
                    ...
                    if (!node.isRoot() && node.getUserObject() != null) {
                        this.setEnabled(
                                isEnabledForHistoryReference(
                                        getSelectedHistoryReferences().get(0)));
                    } else {
                        this.setEnabled(false);
                    }
                }
        ...
        if (display) {
            if (this.isEnabled()
                    && !this.isSafe()
                    && Control.getSingleton().getMode().equals(Mode.protect)) {
