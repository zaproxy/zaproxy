...
class ScannerPauseTimerUnitTest {
    ...
    void shouldExcludePausedTimeFromElapsed() throws InterruptedException {
        // Given - simulate a scan that runs, pauses, waits, then resumes
        scanner.start(new SiteNode(null, 0, "http://example.com"));
        scanner.pause();
        Thread.sleep(50); // Wait 50ms while paused
        scanner.resume();
        // When
        long elapsed = scanner.getElapsedMillis();
        // Then
        // Since we never actually scanned, elapsed should be close to 0
        assertThat("Elapsed time should be small (no actual scanning)", elapsed, lessThan(50L));
    }
